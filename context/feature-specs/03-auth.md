Clerk is already installed and connected. Wire it to the Next.js app: provider, auth pages, redirects, route protection, and user menu.
...
Define public routes using exising sign-in and sign-up env vars. Protect everything else by default. 

Update `/`:
 - authenticated users redirect to `/editor`
 - unauthenticated users redirect to `/sign-in`

Add Clerk's built-in `UserButton` to the editor navbar right seciton for profile settings and logout.

Keep Clerk's default user menu and profile flows intact. Do not rebuild or heavily customize Clerk internals.

Use exiting Clerk env vars. Do not rename or invent new ones. 
