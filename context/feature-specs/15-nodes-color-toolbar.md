Add a small floating color toolbar so selected nodes can change both their backgrounds and text color directly on the canvas.
...
3. Whan a swatch is selected:
 - update both the node background color and text color
 - update the node UI immediately
 - keep this indise the existing collaborative canvas state
 - no server calls
