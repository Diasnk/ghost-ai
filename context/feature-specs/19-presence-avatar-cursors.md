...
- if the editor home and editor canvas use the same navbar components, make sure this presence UI only appears in the canvas/editor room view.
...
- filter the Liveblocks precense list to exclude any entry whose user ID matches the current Clerk user ID
...
- keep the collaborator avatars and the Clerk UserButton the same size so the group looks visually consistent 
- collaborator avatars and display-only, not interactive 
...
- use profile photos when avaliable
- fall back to initials when there is no image
- show up to five collaborator avatars in an overlapping stack 
- show a +N overflow chip when there are more than five collaborators
- add a subtle ring so the avatars stay readable on the dark canvas
...
- update cursors position on React Flow's onMouseMove event
